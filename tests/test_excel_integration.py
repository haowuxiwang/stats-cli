...
class TestDescriptive:
    ...
    def test_descriptive_with_column_name(self):
        result = handler(
            {"command": "descriptive", "params": {"file": xlsx("035_dist_mixed_scales.xlsx"), "column": "medium"}}
        )
        assert succeed(result)

    def test_descriptive_with_column_index(self):
        result = handler(
            {"command": "descriptive", "params": {"file": xlsx("008_threecol_5000rows.xlsx"), "column": "0"}}
        )
        assert succeed(result)

    def test_descriptive_missing_data(self):
        result = handler({"command": "descriptive", "params": {"file": xlsx("038_missing_random_20pct.xlsx")}})
        assert succeed(result)
        if result["status"] == "success":
            assert result["data"]["n"] < 200  # some values were NaN

    def test_descriptive_mostly_missing(self):
        result = handler({"command": "descriptive", "params": {"file": xlsx("039_missing_random_50pct.xlsx")}})
        assert succeed(result)

    def test_descriptive_single_valid(self):
        result = handler({"command": "descriptive", "params": {"file": xlsx("045_missing_single_valid.xlsx")}})
...
class TestOutlier:
    ...
    def test_outlier(self, fname, method):
        result = handler({"command": "outlier", "params": {"file": xlsx(fname), "method": method}})
...
class TestControlChart:
    ...
    def test_control_chart(self, fname, chart_type):
        result = handler({"command": "control_chart", "params": {"file": xlsx(fname), "chart_type": chart_type}})
...
class TestCapability:
    ...
    def test_capability(self, fname):
        result = handler({"command": "capability", "params": {"file": xlsx(fname), "usl": 11.0, "lsl": 9.0}})
...
class TestTrend:
    ...
    def test_trend(self, fname, test_type):
        result = handler({"command": "trend", "params": {"file": xlsx(fname), "test_type": test_type}})
...
class TestClean:
    ...
    def test_clean(self, fname, method):
        result = handler({"command": "clean", "params": {"file": xlsx(fname), "method": method}})
...
class TestTransform:
    ...
    def test_transform(self, fname, method):
        result = handler({"command": "transform", "params": {"file": xlsx(fname), "method": method}})
...
class TestTtest:
    ...
    def test_one_sample_ttest(self, fname):
        result = handler({"command": "ttest", "params": {"file": xlsx(fname), "test_type": "one_sample", "mu": 0}})
...
class TestTimeseries:
    ...
    def test_timeseries(self, fname, analysis_type):
        result = handler({"command": "timeseries", "params": {"file": xlsx(fname), "analysis_type": analysis_type}})
...
class TestReport:
    ...
    def test_report(self):
        result = handler(
            {"command": "report", "params": {"file": xlsx("087_stat_capability.xlsx"), "usl": 11.0, "lsl": 9.0}}
        )
        assert succeed(result)
...
class TestEdgeCases:
    ...
    def test_all_text_file(self):
        ...
        result = handler({"command": "descriptive", "params": {"file": xlsx("048_mixed_all_text.xlsx")}})
        assert result["status"] == "error"
        assert result["error_type"] == "DATA_ERROR"

    def test_mixed_messy_column(self):
        """Messy mixed-type column should handle gracefully."""
        result = handler({"command": "descriptive", "params": {"file": xlsx("050_mixed_messy_column.xlsx")}})
        # Should succeed (numeric values extracted) or fail gracefully
        assert succeed(result)

    def test_inf_values(self):
        """Inf values should be filtered out."""
        result = handler({"command": "descriptive", "params": {"file": xlsx("053_mixed_with_inf.xlsx")}})
        assert succeed(result)

    def test_empty_sheet(self):
        """Empty sheet should fail gracefully."""
        result = handler(
            {"command": "descriptive", "params": {"file": xlsx("073_sheet_empty_plus_data.xlsx"), "sheet": "Empty"}}
        )
        assert result["status"] == "error"
    ...
    def test_wrong_sheet_name(self):
        ...
        result = handler(
            {"command": "descriptive", "params": {"file": xlsx("066_sheet_single.xlsx"), "sheet": "DoesNotExist"}}
        )
        assert result["status"] == "error"

    def test_nonexistent_file(self):
        """Non-existent file should fail gracefully."""
        result = handler({"command": "descriptive", "params": {"file": "/tmp/nonexistent_file.xlsx"}})
        assert result["status"] == "error"

    def test_constant_values(self):
        """Constant values should not crash."""
        result = handler({"command": "descriptive", "params": {"file": xlsx("031_dist_constant.xlsx")}})
        assert succeed(result)
        if result["status"] == "success":
    ...
    def test_single_row(self):
        ...
        result = handler({"command": "descriptive", "params": {"file": xlsx("056_edge_single_row.xlsx")}})
        assert succeed(result)

    def test_duplicate_column_names(self):
        """Duplicate column names should be handled."""
        result = handler({"command": "explore", "params": {"file": xlsx("078_header_duplicate_names.xlsx")}})
        assert succeed(result)
