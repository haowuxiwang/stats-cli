...
def enso_model(x, b1, b2, b3, b4, b5, b6, b7, b8, b9):
    ...
    return (
        b1
        + b2 * np.cos(2 * np.pi * x / b4)
        + b3 * np.sin(2 * np.pi * x / b4)
        + b5 * np.cos(2 * np.pi * x / b7)
        + b6 * np.sin(2 * np.pi * x / b7)
...
class TestNISTNonlinearAll:
    ...
    def test_mgh10(self):
        ...
        try:
            popt, _ = curve_fit(
                mgh10_model, x, y, p0=[0.005, 6000, 350], bounds=([0, 0, 0], [1, 10000, 1000]), maxfev=100000
            )
        except RuntimeError:
