...
EXCLUDE_DIRS = {'__pycache__', '.pytest_cache', '.ruff_cache', '.git', '.github', 'test-output', '.claude', 'excel', 'tests', 'docs'}
EXCLUDE_FILES = {'test_real_data.py', 'stats-cli-py-skill.zip', 'requirements-test.txt', 'build-skill.sh', 'install-skill.sh', '.coverage', 'TODO.md', 'CLAUDE.md', '.pre-commit-config.yaml', '.gitignore', 'pyproject.toml'}
EXCLUDE_EXTS = {'.pyc'}
