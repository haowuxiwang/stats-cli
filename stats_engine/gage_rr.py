...
def _nested(measurements, parts, operators, **kwargs):
    ...
    return {
        ...
        "variance_components": {
            "repeatability": {"variance": r(sigma2_error), "std": r(np.sqrt(sigma2_error))},
            "operator": {"variance": r(sigma2_operator), "std": r(np.sqrt(sigma2_operator))},
            "part_in_operator": {
                "variance": r(sigma2_part_in_op),
                "std": r(np.sqrt(sigma2_part_in_op)),
            },
            "grr": {"variance": r(sigma2_grr), "std": r(sd_grr)},
            "total": {"variance": r(sigma2_total), "std": r(sd_total)},
        },
        "ndc": ndc,
        "rating": rating,
