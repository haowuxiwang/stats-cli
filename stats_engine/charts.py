...
def residual_plot(x, y, slope=None, intercept=None, title="Residual Plot"):
    ...
    # Residuals vs Fitted
    axes[0].scatter(predicted if slope is not None else arr_x, residuals,
                    color='steelblue', alpha=0.6, s=50, edgecolor='white')
...
def weibull_plot(times, shape=None, scale=None, title="Weibull Probability Plot"):
    ...
    if shape is not None and scale is not None and shape > 0 and scale > 0:
        ...
        ax.plot(x_line, y_line, 'r-', linewidth=2,
                label=f'Weibull (β={shape:.2f}, η={scale:.2f})')
...
def ttest_plot(group1, group2=None, p_value=None, ci=None, title="t-Test"):
    ...
    if p_value is not None:
        ...
        y_pos = y_max * 1.1
        ax.annotate(f'p={p_value:.4f} {sig}', xy=(1.5, y_pos),
...
def means_comparison_plot(names, means, ci_lowers, ci_uppers, significant_pairs=None,
                           title="Means Comparison"):
    """Generate means plot with confidence intervals for multiple comparison."""
    fig, ax = plt.subplots(figsize=(10, 5))

    x = range(len(names))
    ax.errorbar(x, means, yerr=[np.array(means) - np.array(ci_lowers),
...
def power_curve(effect_size, n_values, target_power=0.8, title="Power Curve"):
    ...
    ax.axhline(y=target_power, color='red', linestyle='--', linewidth=1.5,
